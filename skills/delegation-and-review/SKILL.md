...
| Cross-model review / second opinion | codex-as-reviewer — for **post-impl review of SMALL code with unstated hazards (the R1 shape)** use **`-m gpt-6-sol -c model_reasoning_effort=medium`** for routine review rounds and **`-m gpt-6-astra -c model_reasoning_effort=medium` as the pre-commit gate** (the last review before committing anything load-bearing, and any change touching numeric/magnitude logic), **not luna** — PROVISIONAL: R1 2026-09-23, pooled N=5, one JS subject, hand-read per finding (astra 15/15 > sol 12–13/15, every sol miss = sumTo precision above 2^53; not significant, Fisher p≈0.17–0.44; gpt-6-luna 3/9 and fabricated an edit 2/3 on h3 — diff the tree); neither is a second family; spec review, rules-file review, security review and large real-repo diffs keep their existing routes (codex model unmeasured there), or grok on STAGED files (agy only pre-implementation — NOT for post-impl review of a real repo) |
