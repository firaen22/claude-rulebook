...
| Spec'd implementation needing judgment, or spanning files | codex (`workspace-write`; when a numeric-magnitude / recursion-depth / precision bound can't be stated: JS → `-m gpt-6-sol -c model_reasoning_effort=medium` WITH the astra pack (owner pick 2026-09-25, scale bench 45/50 vs astra 35/50, N=5; recipe in `workflow_codex_subordinate.md`), alternate / non-JS → `-m gpt-6-astra` medium; within-codex pick only; bound known → spec it for luna), or Agent `general-purpose` sonnet |
...
| Cross-model review / second opinion | codex-as-reviewer — for **post-impl review of SMALL code with unstated hazards (the R1 shape)** use **`-m gpt-5.6-luna -c model_reasoning_effort=medium` WITH the astra pack prefixed** (pack + hash-check recipe in `workflow_codex_subordinate.md`, REVIEW-ROLE UPDATE 2026-09-25), alternates **`-m gpt-6-sol` medium + pack** or **`-m gpt-6-astra` medium**; not bare luna; **JS only** (other languages unmeasured → `gpt-6-astra` medium) — PROVISIONAL: astra-pack bench stage 1, N=5, one held-out JS subject (5.6-luna+pack and sol+pack 15/15 on r1 and r2 after the 09-25 re-grade; bare 5.6-luna 4/15 r2; tied astra on r2 only, at the ceiling); gpt-6-luna fabricated an edit 2/3 on h3 (09-23) — diff the tree; none is a second family; spec review, rules-file review, security review and large real-repo diffs keep their existing routes (codex model unmeasured there), or grok on STAGED files (agy only pre-implementation — NOT for post-impl review of a real repo) |
