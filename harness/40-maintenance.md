...
| `~/.claude/hooks/observe-compaction-events.sh` | YES script; **ASK USER before it does anything but log** | PHASE 0, installed 2026-08-29 with user approval, on PreCompact (manual, auto) and SessionStart (startup, resume, clear, compact). **Measurement only** — appends the raw hook JSON to `~/.claude/session-state/observed.jsonl`, capped at 5 MB, and prints NOTHING on stdout (SessionStart stdout enters the session context). It exists to answer three facts a rejected v3 design assumed: does `session_id` survive a compact; what exact `cwd` string arrives and is it spelled the same in both hooks; which SessionStart matcher fires when. The registered matcher is passed as `$1` because the hook JSON does not report it. Fixture-tested 5/5 before install. **Turning this into a hook that preserves, injects, or recovers anything is a NEW design and goes back to ASK USER** — four reviewers rejected the version that did. |
