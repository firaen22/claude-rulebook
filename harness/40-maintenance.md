...
| `~/.claude/hooks/observe-compaction-events.sh` | YES script; **ASK USER before it does anything but log** | PHASE 0 measurement hook. **Measurement only** — writes ONE file per event into the `~/.claude/session-state/observed/` DIRECTORY (`<ns>-<pid>.complete.json`, `.truncated.json`, `.error.txt`), advisory caps 5 MB / 2000 files, and prints NOTHING on stdout (SessionStart stdout enters the session context). **Corrected 2026-09-01** — this row described the pre-v22 hook, which appended to a single `observed.jsonl`; that file still exists and the hook itself calls it legacy. v22 (installed 2026-09-01, md5 `d1f0a874`) also never deletes: no `unlink`/`rename`/`ftruncate` anywhere, every file created `O_EXCL|O_NOFOLLOW`. Contract harness + its 13-mutant validation gate: `claude-code-technique` scratchpad `round23/harness/` (`./run.sh sturdy <hook>`). **Turning this into a hook that preserves, injects, or recovers anything is a NEW design and goes back to ASK USER** — four reviewers rejected the version that did. Install date, matcher inventory, the three questions it answers, and the corrected `$1` rationale: `41-file-registry.md`. |
...
| `~/.claude/hooks/harness/cache_gate/` (cachelib · build_cache · check_caches · check_catalog · cache_manifest.json · pre-commit) | YES script; ASK USER to remove/disable | The harness's own commit-time gates. `check_caches` (P1b): pins each distilled cache's SOURCE shas + its own BODY sha; exit 0 fresh / 1 drift / 2 error. `check_catalog` (P1a): §1↔41 completeness + verdict-parity. The committed `pre-commit` hook runs BOTH on the STAGED INDEX (`git checkout-index`); `scripts/run_all.sh` runs BOTH on the worktree. Re-verify after ANY edit: `bash ~/.claude/hooks/harness/scripts/run_all.sh` + the P1a/P1b batteries. Fail-closed idioms: ROOT-from-file-location, resolve-within-root, GateError→exit 2. `check_catalog` prints its governed-scope patterns + special-cased objects on every run; the §1 verdicts it checks are mirrored per-object in `41-file-registry.md`. |
