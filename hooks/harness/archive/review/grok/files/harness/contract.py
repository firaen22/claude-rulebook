...
def build_home(home, variant=""):
    ...
    if variant == "obs_is_file":
        shutil.rmtree(obs); open(obs, "wb").write(b"i am a file, not a dir\n")
...
def cases():
    ...
    A(("B06_path_hostile",      "run", ["manual"],  good, {"PATH": "@HOSTILE@"}, "", "python3 stub prints to stdout"))
...
def run_case(target, case, workdir):
    ...
    try: os.chmod(os.path.join(home, ".claude", "session-state", "observed"), 0o755)
    except OSError: pass
    ...
    return {"name": name, "note": note, "rc": rc, "stdout_bytes": so, "stderr_bytes": se,
            ...
            "within_ceiling": elapsed < WALL_CEIL - 0.5 and rc != "TIMEOUT",
            "kind": kind, "alive_at_signal": alive_at_signal,
            "destroyed": destroyed}
...
def main():
    ...
    for c in cs:
        ...
        print(f"{'FAIL' if r['fails'] else 'ok  '} {r['name']:<24} rc={str(r['rc']):>7} "
              f"out={r['stdout_bytes']:<6} {r['elapsed']:>6.2f}s  {'; '.join(r['fails'])}")
