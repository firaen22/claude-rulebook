...
# M18 (2026-09-05): double-fork + setsid + execv a macOS PLATFORM binary, fds closed.
# ps -E emits no environment for /bin/sleep, so the HOME= survivor clause never
# sees it; only cwd (inherited, survives execve) attributes it to this run.
/usr/bin/python3 -c "
import os
if os.fork()==0:
    os.setsid()
    if os.fork()==0:
        os.closerange(0,64)
        os.execv('/bin/sleep',['/bin/sleep','25'])
    os._exit(0)
os.wait()
" 2>/dev/null
