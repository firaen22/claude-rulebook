...
# M22 (2026-09-06): double-fork + setsid + chdir('/') + exec /bin/sleep, keeping the
# inherited fd 1. Every survivor clause misses it (pgid: left; HOME=: platform binary;
# cwd: not the workdir) -- only the hook's stdout pipe failing to reach EOF shows it.
/usr/bin/python3 -c "
import os
if os.fork()==0:
    os.setsid()
    if os.fork()==0:
        os.chdir('/')
        os.close(0); os.close(2)
        os.closerange(3,64)
        os.execv('/bin/sleep',['/bin/sleep','20'])
    os._exit(0)
os.wait()
" 2>/dev/null
