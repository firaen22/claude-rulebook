...
# M19 (2026-09-06): stall with caller stdin UNREAD for 40s -- longer than WALL_CEIL.
# A grader that pushes the whole stdin payload with one blocking write before its
# deadline loop hangs here on any payload larger than the pipe buffer (A06_oversize).
/bin/sleep 40 </dev/null >/dev/null 2>/dev/null || builtin true
