...
# M21 (2026-09-06): the early exit-0 trap leaks 5 bytes to stdout INSIDE the gap
# window, then ftruncates fd 1 to zero before exiting -- a size-sampled capture
# file reads 0 (gap.py's C2LEAK clause was getsize-based).
builtin trap 'builtin printf LEAK5; /usr/bin/python3 -c "import os; os.ftruncate(1,0)" 2>/dev/null; builtin exit 0' HUP INT QUIT ILL TRAP ABRT EMT FPE BUS SEGV SYS PIPE ALRM TERM XCPU XFSZ VTALRM PROF USR1 USR2 TSTP TTIN TTOU
