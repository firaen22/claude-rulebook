...
def cases():
    ...
    A(("H02_interp_all_fail",   "interp",   ["manual"], good, {"__V": "fail"}, "fail", "every candidate fails the 1337 probe"))
...
def run_case(target, case, workdir):
    ...
    if os.path.exists(ext_root): shutil.rmtree(ext_root)
    ...
    orphans = 0
    try:
        ps = subprocess.run(["/bin/ps", "-o", "pid=,pgid=,state=", "-ax"],
