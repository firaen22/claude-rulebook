...
# --- parser false positives + newline scoping (2026-09-01) -------------------
# The hook blocked its own maintenance edit: a heredoc body's apostrophes broke
# shlex, and the degraded raw scan's unbounded DOTALL span linked an unrelated
# `rm -rf "$TDIR"` to an `api_key` many lines later. All FP cases below returned
# 2 before the fix; all TP cases must keep returning 2.

# The mechanism that blocked this hook's own maintenance edit on 2026-09-01:
# an unbalanced quote anywhere (a heredoc body's apostrophe) makes shlex fail, and
# the degraded scan's old unbounded DOTALL span then linked an `rm` on one line to
# an `api_key` on another. Verified to return 2 on the pre-fix hook.
assert_exit 0 "the 2026-09-01 blocked maintenance edit (unbalanced quote, tokens on separate lines)" \
  $'echo don\'t\nrm -rf tmp\necho API_KEY=abcdef0123456789'  # noqa: sync-guard -- fixture, not a key

# heredoc bodies are DATA, not commands
assert_exit 0 "heredoc prose mentioning rm + .key (FP)" \
  $'cat <<\'DOC\' > notes.md\nDon\'t rm the server.key by hand; rotate it.\nDOC'
assert_exit 0 "heredoc payload with apostrophe + far-apart tokens (FP)" \
  $'python3 - <<\'PY\'\n# the guard\'s harness makes a temp dir\nrun(["rm","-rf",tdir])\ntext = "API_KEY=abcdef0123456789"\nPY'  # noqa: sync-guard -- fixture, not a key
assert_exit 0 "indented heredoc <<- body stripped" \
  $'cat <<-\'EOF\'\n\trm server.key by hand\n\tEOF'
assert_exit 0 "two heredocs opened on one line" \
  $'diff <(cat <<\'A\'\nrm secret.pem\nA\n) <(cat <<\'B\'\nrm id_rsa\nB\n)'
assert_exit 0 "unterminated heredoc swallows rest, as the shell does" \
  $'cat <<\'EOF\'\nrm ~/.ssh/id_rsa\n'
# ...but the line that OPENS the heredoc is still a real command
assert_exit 2 "heredoc-introducing line still scanned" \
  $'rm ~/.aws/credentials <<\'EOF\'\nharmless\nEOF'

# a newline is a command separator, not whitespace
assert_exit 0 "later line is not an argument of an earlier verb (FP)" \
  $'rm -rf "$TDIR"\necho API_KEY=abcdef0123456789'  # noqa: sync-guard -- fixture, not a key
assert_exit 2 "destructive command on its own line" \
  $'rm notes.txt\nrm .env'
assert_exit 2 "override must NOT leak across a newline" \
  $'CRED_GATE_APPROVED=1 rm .env\nrm ~/.ssh/id_rsa'
assert_exit 0 "override still covers its own line" \
  $'CRED_GATE_APPROVED=1 rm .env\nls -la'
assert_exit 2 "CRLF line endings still separate" \
  $'rm notes.txt\r\nrm .env'
assert_exit 2 "newline inside quotes is not a separator" \
  $'rm "a\nb/credentials.bak"'

# the degraded raw scan is bounded to one line and RAW_MAX_SPAN chars
assert_exit_raw 2 "unparseable, verb and token adjacent" \
  '{"tool_input": {"command": "echo \"unclosed && rm ~/.aws/credentials"'
assert_exit_raw 0 "unparseable, verb and token on different lines" \
  '{"tool_input": {"command": "echo \"unclosed\nrm build/cache\nnote: api_key rotation"'
