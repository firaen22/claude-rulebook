...
# Strip BASH_ENV/ENV from the environment before anything else runs.
# settings.json invokes this hook as `bash -p`, which stops the PARENT shell
# from sourcing $BASH_ENV at startup — but that flag does not propagate: the
# gate run at the bottom of this file is a plain `bash "$gates"` child, and a
# child bash re-reads BASH_ENV from the inherited environment. Measured
# 2026-08-31: with `bash -p` alone, a `git commit` call still executed an
# attacker-supplied BASH_ENV script inside the gate run itself — i.e. on the
# exact path this hook exists to police. Unsetting here removes it from the
# environment every child inherits, so the flag and this line together close
# both halves. Unsetting an already-unset name is not an error under `set -u`.
unset BASH_ENV ENV
