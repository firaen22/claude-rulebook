...
> **⤷ SCALE-EXCEPTION UPDATE 2026-09-25 (owner-approved; supersedes the astra pick above): unstated-bound implementation → `gpt-5.6-luna` medium WITH the astra pack prefixed; `gpt-6-astra` medium = alternate.** Evidence: astra-pack bench stage 2, N=5, PREREG D5 fired — 5.6-luna+pack e4 scale 15/15, total 30/30 (bare 5.6-luna 10/15), all 5 util.js re-graded by hand (closed-form sumTo, iterative flatten). Do NOT use the pack with gpt-6-sol here (scale 15/15 → 11/15, D7 harm) or gpt-6-luna (fabricated a fix 1/5 on h3). Same recipe as the REVIEW-ROLE UPDATE 2026-09-25 below. PROVISIONAL: one task family, JS only, pack as prompt prefix. → [[finding-astra-pack-stage1-2026-09-24]] (project memory).
