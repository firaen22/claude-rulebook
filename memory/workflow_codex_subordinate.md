...
> **ASTRA EFFORT BAND (2026-09-06, user policy): astra runs low–medium, DEFAULT low.** Measured: astra low == astra medium on every de-saturating axis (E4 scale 9/9 both; review 4/4 both) — low is cheaper for identical output, so pick low; go medium only to buy the ceiling on a high-stakes run. Never astra high+ (no measured gain, and pricing is ~2–3× sol/tier — see pricing datum in [[finding-codex-56-family-2026-07-22]]). Same low==medium equivalence holds for luna.
> **[HISTORY for JS — superseded by the SCALE-ROUTE UPDATE 2026-09-25 below; still the pick outside JS]** **MODEL ROUTING EXCEPTION — `gpt-6-astra` for UNSTATED-SCALE work (2026-09-06, Test 6 after self-review).** One of two measured axes where codex models differ (the other is the review role, next block). E4 probe (write hasDuplicate/flatten/sumTo from a happy-path spec, graded on UNSTATED scale in timeout-guarded children): **luna scale 4/9 — `sumTo(1e9)` as a loop 3/3 (returns 500000000067109000, silently off; correct 500000000500000000), deep `flatten` recursive 2/3 (stack overflow); astra 9/9 — closed form, iterative.** Direct re-execution, N=3. Everything else (H1/H2/H3, effort tier, E1 beyond-the-brief) SATURATES for astra exactly as for luna.
...
> **⤷ SCALE-ROUTE UPDATE 2026-09-25 (CURRENT for JS; owner override of the PREREG default):** implementing JS from a spec that states no magnitude/depth/precision bound → **`-m gpt-6-sol -c model_reasoning_effort=medium` WITH the astra pack prefixed** (same recipe + hash check as the REVIEW-ROLE UPDATE 2026-09-25 below); alternate `-m gpt-6-astra` medium. Evidence: pre-registered scale bench (N=5, astra same-run, `experiments/astra-scale-bench-2026-09-25/`, merged PR #3 `608d5dd`): scale sol+pack **45/50** vs astra 35, luna+pack 35, sol 33, luna 24; exact probes past 2^53 silently wrong sol+pack 5/20 vs astra 15/20. **No arm met the PREREG bar** (held-out e5 ≥22/25; sol+pack 21/25), so this route is the OWNER's call on the evidence, not a PREREG decision. The e4 closed-form 'astra 9/9' above passed partly by luck (sumTo(1e9) is exactly representable; at 1e9+1 astra is wrong 5/5). Pack costs ~+35–55% tokens and 1.5–2× wall time. **Whatever model implements: STATE exactness past 2^53 in the spec** (no arm but sol+pack did it unprompted, and it too missed 5/20). Would change this: a rerun where sol+pack falls below astra, or a non-JS / non-precision task family (unmeasured).

> **[HISTORY — superseded for JS by the REVIEW-ROLE UPDATE 2026-09-25 below; still the review pick outside JS]** **REVIEW-ROLE MODEL (2026-09-06, Test 7, corrected by hand-count): post-implementation code review → `-m gpt-6-astra -c model_reasoning_effort=medium`; not luna.** PROVISIONAL preference, not a prohibition: N=3/model, one JS subject, medium effort, unstated-hazard design — spec review, rules-file review, security review and large real-repo diffs were NOT measured. R1 probe (review spec-correct code carrying 10 execution-verified unstated defects): distinct-defect coverage **astra 9/10 in every run, 0 nits**; sol 8/10 ×3 but pads 2–3 validation/reversed-bounds nits per run and takes ~2× wall time (113–182s) — its apparent breadth was bullet inflation; luna 5/8/6, high variance, weakest. astra also fastest (76–85s) and fewest tokens (17–19K). sol's only residual: it surfaced 2 oddball true finds in 3 runs (ß→SS in initials; sparse-array holes in hasDuplicate) that astra never did → **sol = optional ADDITIONAL codex pass only when a miss is catastrophic and time is free**, not a primary reviewer and not a replacement for grok as the second-family lens (routing map §2). astra never found ß; sol never found flatten's quadratic copy. luna stays the IMPLEMENTATION default (R1 measures review only; luna's one implementation weakness is the separate E4 exception above). The cross-model-review skill owns packet + verdict check; this line only picks the model. N=3, one subject, token≠money, drift shelf-life — replicate at N≥5 on a second subject before treating as permanent. Harness: `experiments/codex-hardaxis-2026-09-04/r1/`.
> **[HISTORY — superseded for JS by 2026-09-25 below]** **⤷ REVIEW-ROLE UPDATE 2026-09-23 (gpt-6 bench + R1 top-up, codex 0.155.1, medium): `gpt-6-sol` = routine review rounds; `gpt-6-astra` = pre-commit gate** (last review before committing anything load-bearing, and any change touching numeric/magnitude logic) — split set by owner 2026-09-23. Pooled N=5 on the same R1 subject, hand-read per finding: **astra 15/15 > sol 12–13/15** — every sol miss is `sumTo`'s precision loss above 2^53 (t2/t4 name only `Infinity`/fractional input, t1 only a counter stall at 2^53); flatten depth and O(n²) 5/5 for both. Gap not significant (Fisher p≈0.17–0.44), so the gate, not the default, carries astra's edge. (A same-day "sol 15/15 = astra → sol default" was a grader leak — `grade.js` credited sumTo from a "floating point" remark on the toCents line; fixed lab `efae106`, retracted.) sol is not cheaper on review either (tokens median 19.6K vs astra 17.8K, astra spikes ~30K in 2/5; latency median 83 vs 74 s). ONE JS subject. **gpt-6-luna: do not use for review (R1 3/9 per finding) and diff the tree after any gpt-6-luna edit** — on h3 it claimed "changed … now prints 14.58" in 2/3 runs with only read commands in its event log and the file unchanged; codex's final message is never evidence of an edit. Implementation default stays `gpt-5.6-luna`; astra E4 scale exception stays. Harness: `experiments/codex-gpt6-bench-2026-09-23/` → [[finding_codex_gpt6_bench_2026-09-23]].
>
> **⤷ REVIEW-ROLE UPDATE 2026-09-25 — CURRENT (supersedes the 09-23 sol/astra split for JS; owner-approved, corrected same day): R-shape review of JS → `gpt-5.6-luna` medium WITH the astra pack prefixed; alternates `gpt-6-sol` medium + pack or `gpt-6-astra` medium** (luna quota-blocked, or a second pass when a miss is catastrophic). Not bare luna (held-out r2 4/15). Outside JS: unmeasured — use `gpt-6-astra` medium. Evidence: astra-pack bench stage 1, N=5, PREREG D4 — 5.6-luna+pack r1 15/15, held-out r2 15/15; bare 5.6-luna 11/15, 4/15. A five-lens cross-model review (gpt-6-sol, Fable, grok-4.7, gpt-6-astra, TypeSafe) + re-grade corrected two claims: sol+pack is ALSO 15/15 on r1 and r2 (the reported "no lift 12→11" was the grader missing 'nested…RangeError'), and astra ran in that bench on r2 only, so the result is **tied astra on r2 only**, at the score ceiling — not "equals astra". 0 grader-flagged fp on r2 (r1: 1 flag). Graders credit keywords, not mechanisms; the luna+pack r2 cells were hand-read and are real. gpt-6-luna+pack: not adopted (r1 13/15, 2 fp; fabricated an h3 fix 1/5). Recipe — the pack is on `claude-code-technique` main (PR #2, merge `cfbf423`); pull it by the pinned commit and check the hash:
> `git -C "/Users/yauch/Documents/claude code technique" show df7404e:astra-pack/AGENTS.md > "$TMPDIR/astra-pack.md" && shasum -a 256 "$TMPDIR/astra-pack.md"` → must print `4a15952a48a9bbeea0ab52a8eefb8ddfbbd9406002ce0f928cab244ffcc286b6`; then prompt = `$(cat "$TMPDIR/astra-pack.md"; printf '\n\n---\n\n'; cat brief.md)`. The pack was tested ONLY as a prompt prefix (never installed as AGENTS.md) and on top of `~/.codex/AGENTS.md`. Its closing provenance line ("UNMEASURED until that bench runs") is stale; it is kept so the hash pin holds. PROVISIONAL: two tiny JS subjects, one held out. The unstated-SCALE implementation exception did NOT move: a same-day luna+pack move was reverted (D5 rested on grader artifacts); `experiments/astra-scale-bench-2026-09-25` decides. → [[finding-astra-pack-stage1-2026-09-24]] (project memory).
>
> **⤷ PARTIALLY SUPERSEDED 2026-07-23: the "MIX MODE DEAD / sol offers nothing" half STANDS for implementation (sol's only measured residual is the optional review second pass, 2026-09-06); the "codex default stays gpt-5.5" half is REPLACED by luna-default (see PROMOTED block above — luna was re-tested on these same H1/H2 axes and tied 5.5); the "no tier routing within codex" half is REPLACED 2026-09-06 by luna-default + the two astra exceptions (E4 unstated scale — still current; R1 review — superseded for JS 2026-09-25) above.** **HARD-AXIS FOLLOW-UP (2026-07-10, 12/12 runs): sol offers NOTHING over 5.5 even at max reasoning — MIX MODE DEAD.** Arms 5.5-med / sol-med / sol-max × the two non-saturated frontier axes (H1: LRU cache impl incl. unstated capacity-0/negative edges, hang-detected; H2: localize a planted `arr.sort()` lexicographic bug in `median()` among 5 functions) × 2 reps. ALL arms 12/12 perfect — every LRU survived cap-0/negative (the edge that hung even claude 2/3 in the frontier bench, on free-tier models), every localization named median + mechanism + verified failing input (hand-checked: 5.5 ran the code itself as evidence). **sol-max costs 2–4× wall time (194–248s vs 51–107s) for zero gain.** VERDICT: codex default stays gpt-5.5, no tier routing within codex, no mix mode. Codex-class saturates these axes — the frontier-bench failure rates were a free-model phenomenon. Would change conclusion: a probe set that de-saturates at codex tier (multi-file integration bugs, adversarial specs), or a real-work miss by 5.5 that sol catches. Sibling to [[workflow-agy-subordinate]] — Codex is a strong code writer; delegate implementation, not just lookups. (It TIES agy on implementation of well-specified tasks per the 2-round experiment — its edge is low run-to-run variance + tighter typing, not raw "stronger." See capability profile.)
