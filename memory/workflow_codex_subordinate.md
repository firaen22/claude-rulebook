...
> **⤷ REVIEW-ROLE UPDATE 2026-09-23 (gpt-6 bench + R1 top-up, codex 0.155.1, medium): R1-shape review default → `-m gpt-6-sol -c model_reasoning_effort=medium`; astra = alternate.** Pooled N=5 on the same R1 subject: **sol 15/15 = astra 15/15**; pre-registered tie rule picks sol for lower variance — NOT for cost or speed (review tokens median sol 19.6K vs astra 17.8K, astra spikes ~30K in 2/5; latency median 83 vs 74 s). The 09-06 "sol pads nits / ~2× slower" read above was gpt-6-era sol at N=3 and did not reproduce. Still ONE JS subject at ceiling — a second R1 subject where they diverge would change this. **gpt-6-luna: do not use for review (R1 6/9) and diff the tree after any gpt-6-luna edit** — on h3 it claimed "changed … now prints 14.58" in 2/3 runs with only read commands in its event log and the file unchanged; codex's final message is never evidence of an edit. Implementation default stays `gpt-5.6-luna`; astra E4 scale exception stays. Harness: `experiments/codex-gpt6-bench-2026-09-23/` → [[finding_codex_gpt6_bench_2026-09-23]].
>
