...
> 📋 **Membership-only refresh 2026-09-06 (liveness/listing check, NOT a bench — no scores below are new).** `opencode models | grep '^opencode/'`: the 6-model free core from 08-29 (big-pickle, mimo-v2.5-free, muse-spark-1.2-contributor-free, nemotron-3-ultra-free, nemotron-3.5-lightning-free) is present MINUS `hy3-free`, which is **still absent** — confirms the 08-29 same-session delisting did not clear over the following days. New, unbenched arrivals: `opencode/ling-3.0-flash-fin-free` (distinct from the long-delisted `ling-3.0-flash-free` — do not assume its old scorecard applies), `opencode/muse-spark-1.3-contributor-free` (a version-bumped sibling of the current best free model — bench before trusting, 1.2's scorecard does not transfer), `opencode/deepseek-v4-flash-vision-exp` (new deepseek variant, vision-tagged — likely same payment-gate as its siblings but not probed). `opencode/deepseek-v4-flash` and `opencode/deepseek-v4-pro` remain listed and still presumed payment-gated (not re-probed this pass). **Action: none of the 3 new arrivals have been benched or probed — do not route to them assuming any inherited score.**
