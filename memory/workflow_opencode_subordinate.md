...
| ⛔ **NOT SERVING as of 2026-08-21** — listed in `opencode models` but every call fails rc=1 in ~0.6s, 0 bytes, server-side `UnknownError`/"Unexpected server error". 4/4 failures on a trivial PONG while an **interleaved big-pickle control PONG'd 3/3 on the same transport in the same window** → provider-side, not capability, not local. **Do NOT record 0/10** — an outage and an incapacity are the same shape in a scorecard; only the control separates them. Re-probe before writing it off; do not re-bench from the failed run. Prior score below is historical. | `opencode/deepseek-v4-flash-free` | **Re-benched 2026-08-04 (N=2×4): 8/10.** P1 2/2, P2 basic 2/2, P3 2/2 (caught mechanism + counterexample — one verdict flipped from a grader false-MISS, see soundness note below), P4 no-FP 2/2. **Unstated `size=0` → EDGE-LOOP 2/2**, same blind spot as ling/laguna/mimo. Rate-limit caveat still stands. Roams cwd. |
